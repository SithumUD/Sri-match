...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fdf8f4] px-4 pt-8 pb-16 font-['DM_Sans'] text-[#2d1810] sm:px-6">
      <Helmet>
        <title>Discover Matches | SriMatch</title>
        <meta name="description" content="Find your perfect life partner in Sri Lanka. Search through verified profiles by district, religion, and more." />
      </Helmet>
      <SearchHeader searchTerm={searchTerm} onSearchChange={setSearchTerm} />

      <div className="mx-auto mt-8 grid max-w-[1200px] grid-cols-1 items-start gap-7 lg:grid-cols-[272px_1fr]">
        <FilterSidebar 
          filters={filters}
        ...
        <main>
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            <p className="text-[0.85rem] text-[#9a7060]">
              <strong className="text-[#2d1810]">{totalElements}</strong> {totalElements === 1 ? "profile" : "profiles"} found
            </p>
            <div className="flex flex-wrap gap-1.5">
              {SORT_OPTIONS.map(s => (
                <button 
                  key={s.key} 
                  className={`flex items-center gap-1 rounded-full border-[1.5px] px-3 py-1.5 font-['DM_Sans'] text-[0.74rem] font-medium transition-all duration-200 ${sortOrder === s.key ? "border-transparent bg-gradient-to-br from-[#3d1f12] to-[#8b4e2e] text-white shadow-[0_4px_10px_rgba(139,78,46,0.22)]" : "border-[#e8ddd8] bg-[#fdf8f4] text-[#6b4a3a] hover:border-[#c9856a]"}`}
          ...
          {status === 'loading' ? (
            ...
            <div className="rounded-[20px] bg-white px-8 py-16 text-center shadow-[0_8px_28px_rgba(120,60,30,0.06)]">
              ...
              <p className="text-[0.85rem] text-[#9a7060]">Try adjusting your filters or search terms to see more profiles.</p>
            </div>
          )}
