...
const AdminService = {
    ...
    // Locations (Cities)
    getAdminCities: () => {
        return API.get('/admin/locations/cities'); // Note: Backend has no GET all in AdminLocationController yet? 
        // Wait, let's check AdminLocationController again.
    },
    ...
    // Audit Logs
    getAuditLogs: (email = '') => {
        return API.get(`/admin/audits${email ? `?email=${email}` : ''}`);
    },
