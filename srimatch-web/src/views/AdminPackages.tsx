...
import {
  ...
  SparklesIcon, TrashIcon, TrendingUpIcon, VideoIcon,
} from "lucide-react";
...
const AdminPackages = () => {
  ...
  const [tiktokPackages, setTiktokPackages] = useState([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await AdminService.getAdminPackagesOverview();
      if (res && res.data) {
        setPackages(res.data.packages || []);
        setBoostPackages(res.data.boostPackages || []);
        setBankDetails(res.data.bankDetails || []);
        setTiktokPackages(res.data.tiktokPackages || []);
      }
    } catch (err) {
  ...
  const handleToggleTikTok = async (id) => {
    try {
      await TikTokService.adminTogglePackage(id);
      setTiktokPackages(prev => prev.map(p => p.id === id ? { ...p, active: !p.active } : p));
    } catch (err) { alert("Failed to toggle status"); }
  };
  ...
  const openModal = (type, item = null) => {
    ...
    if (type === "package") {
      ...
      setFormData(item ? { ...item } : { name: "", description: "", price: "", durationDays: 1, offerPercentage: 0, active: true });
    } else {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      } else if (showModal === "tiktok") {
        if (editingItem) await TikTokService.adminUpdatePackage(editingItem.id, formData);
        else await TikTokService.adminCreatePackage(formData);
      } else {
    ...
    <>
      ...
      <div className="ap2-root">
        ...
        <div className="ap2-header">
          ...
          <button className="ap2-btn-primary" onClick={() => openModal(activeTab === "packages" ? "package" : activeTab === "boost" ? "boost" : activeTab === "tiktok" ? "tiktok" : "bank")}>
            <PlusIcon size={16} /> Add {activeTab === "packages" ? "Package" : activeTab === "boost" ? "Boost Package" : activeTab === "tiktok" ? "TikTok Package" : "Bank Detail"}
          </button>
        </div>
        ...
        <div className="ap2-tabs">
          ...
          <div className={`ap2-tab ${activeTab === "tiktok" ? "active" : ""}`} onClick={() => setActiveTab("tiktok")}>
            <VideoIcon size={15} /> TikTok Packages
          </div>
              ...
              tiktokPackages.map(pkg => (
                <div key={pkg.id} className={`ap2-card ${!pkg.active ? 'inactive' : ''}`}>
                  ...
                  <div className="ap2-card-hdr">
                    <h3 className="ap2-card-title">{pkg.name}</h3>
                    <div className="ap2-card-duration">
                      <VideoIcon size={12} /> {pkg.durationDays} {pkg.durationDays === 1 ? 'Day' : 'Days'} Feature
                    </div>
                  </div>
                  <div className="ap2-card-price-row">
                    <span className="ap2-card-price">Rs {Number(pkg.price).toLocaleString()}</span>
                    {pkg.offerPercentage > 0 && (
                      <span className="ap2-card-off">-{pkg.offerPercentage}% OFF</span>
                    )}
                  </div>
                  <p className="ap2-card-desc">{pkg.description}</p>
                  <div className="ap2-card-footer">
                    <label className="ap2-toggle">
                      <input type="checkbox" checked={pkg.active} onChange={() => handleToggleTikTok(pkg.id)} />
                      <span className="ap2-slider"><span className="ap2-knob" /></span>
                    </label>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button className="ap2-btn-icon" onClick={() => openModal("tiktok", pkg)}><Edit3Icon size={14} /></button>
                    </div>
                  </div>
                </div>
              ))
              ...
              <form onSubmit={handleSave}>
                <div className="ap2-modal-body">
                  {showModal === "package" ? (
                    ...
                    <>
                      <div className="ap2-form-group">
                        <label className="ap2-label">Package Name</label>
                        <input className="ap2-input" value={formData.name || ""} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. 3 Days TikTok Spotlight" required />
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div className="ap2-form-group">
                          <label className="ap2-label">Price (LKR)</label>
                        ...
                        <div className="ap2-form-group">
                          <label className="ap2-label">Duration (Days)</label>
                          <input type="number" min="1" className="ap2-input" value={formData.durationDays || 1} onChange={e => setFormData({...formData, durationDays: e.target.value})} required />
                        </div>
                      </div>
                      <div className="ap2-form-group">
                        <label className="ap2-label">Offer Percentage (%)</label>
                        <input type="number" min="0" max="100" className="ap2-input" value={formData.offerPercentage || 0} onChange={e => setFormData({...formData, offerPercentage: e.target.value})} />
                      </div>
                      <div className="ap2-form-group">
                        <label className="ap2-label">Description</label>
                        <textarea className="ap2-input ap2-textarea" value={formData.description || ""} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="What this spotlight includes..." />
                      </div>
                    </>
                  ) : (
                    <>
